...
def create_coq_backend(backend_type: str | None = None) -> CoqBackend:
    ...
    if backend_type == "coqlsp":
        from wily_rooster.extraction.backends.coqlsp_backend import CoqLspBackend  # type: ignore[import-not-found]
        return CoqLspBackend()

    if backend_type == "serapi":
        from wily_rooster.extraction.backends.serapi_backend import SerAPIBackend  # type: ignore[import-not-found]
