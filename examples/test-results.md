...
| 1.2 | Which lemmas in stdlib mention both Nat.add and Nat.mul? | FAIL | search_by_symbols returned raw [id, score] pairs (e.g. [354, 1.0]) with no lemma names — output not human-readable |
| 1.3 | Search for lemmas with type forall n : nat, n + 0 = n | PASS | search_by_type returned results including Coq.Init.Peano.plus_n_O |
| 1.4 | Find a lemma of type List.map f (List.map g l) = List.map (fun x => f (g x)) l | FAIL | search_by_type returned 50 raw [id, score] pairs without resolving to lemma names — output is unreadable internal IDs |
| 1.5 | Find all commutativity lemmas in MathComp — anything matching _ * _ = _ * _ | PASS | search_by_structure returned 50 structurally similar results with decl_id and scores |
| 1.6 | Find lemmas concluding with _ + _ <= _ | PASS | search_by_structure returned 50 structurally similar results with reasonable scores |
| 1.7 | What rewrites exist for Nat.add n 0? | PASS | search_by_name returned Nat.add_0_r, Nat.add_0_l, and Nat.eq_add_0 |
...
| # | Prompt | Result | Reason |
|---|--------|--------|--------|
| 3.1 | Show me the full definition of Coquelicot.Derive.Derive | PASS | list_modules returned 16 Coquelicot modules; search_by_name found Coquelicot.Complex.C_derive and related items |
| 3.2 | Which module gives me access to ssralg.GRing.Ring? | PASS | list_modules returned 21 mathcomp modules including boot and order packages |
| 3.3 | What is the body of MathComp.ssrnat.leq? | PASS | get_lemma returned mathcomp.boot.ssrnat.leq with type nat -> nat -> bool, dependencies, and 11 dependents |
...
| 3.6 | What Proper instances are registered for Rplus in Coquelicot? | PASS | list_instances with fully qualified "Coq.Classes.Morphisms.Proper" returned 75+ instances (Nat.add_wd, Nat.mul_wd, etc.) |
| 3.7 | What lemmas are in the arith hint database? | PASS | inspect_hint_db returned valid structured response for "core" database (0 entries in minimal context, but tool functions correctly) |
| 3.8 | What's in the Coq.Arith module? | PASS | list_modules returned 13 submodules including PeanoNat (966 declarations) |
...
| 3.10 | Show me the dependency graph around Nat.add_comm | FAIL | visualize_dependencies returned graph with only 1 node (root) and no edges — dependency data not populated |
...
| 4.8 | Try applying intros then induction n in my current proof session | PASS | Both tactics submitted successfully via submit_tactic; produced base and inductive subgoals |
| 4.9 | Step through the proof of add_comm in examples/arith.v | PASS | Opened session, stepped through 2 tactics (intros n m, apply Nat.add_comm), extracted full 3-state proof trace |
...
| 4.12 | Visualize the proof tree for app_nil_r in examples/lists.v | PASS | visualize_proof_tree returned full Mermaid flowchart with 6 steps showing branching from induction |
| 4.13 | Render the step-by-step proof evolution of modus_ponens in examples/logic.v | PASS | visualize_proof_sequence returned 4 Mermaid diagrams showing proof evolution with CSS annotations |
...
| 6.2 | What typeclasses does std++ provide for finite maps? | PASS | list_modules returned 48 stdpp modules; list_typeclasses returned non-empty list |
| 6.3 | /check-compat | SKIP | Slash command |
| 6.4 | What Coq packages are currently installed? | PASS | query_packages returned installed opam packages (coq 9.1.1, coq-coquelicot 3.4.4, coq-mathcomp-ssreflect 2.5.0, etc.) |
...
- **intermediate value theorem** (Coquelicot) not indexed despite the library being indexed
- **MathComp ssralg not indexed** — only mathcomp.boot.* and mathcomp.order.* present; GRing.Ring and algebra modules missing
