...
- ENSURES: Sends `Print Instances <typeclass_name>.` to the session backend via the Proof Session Manager's `submit_command` operation (routed through the session's CoqBackend per [proof-session.md](proof-session.md) §4.4.1). Parses the response into a list of TypeclassInfo records. Returns the list, which may be empty.
...
- ENSURES: Sends `Print Typeclasses` to the session backend via the Proof Session Manager's `submit_command` operation (routed through the session's CoqBackend per [proof-session.md](proof-session.md) §4.4.1). Parses the response into a list of records, each with typeclass name and instance count. For each typeclass, a follow-up `Print Instances <class>` is issued via `submit_command` to obtain the instance count.
- MAINTAINS: The session's proof state is unchanged after the call.
...
| Property | Value |
|----------|-------|
| Operations used | `submit_command` (for `Print Instances`, `Print Typeclasses`, `Set Typeclasses Debug Verbosity 2`, `Unset Typeclasses Debug` — routed through the session's CoqBackend per [proof-session.md](proof-session.md) §4.4.1) |
