...
9. Proof-body detection: For each declaration whose kind ∈ {`lemma`, `theorem`, `definition`, `instance`}, determine `has_proof_body` using four signals evaluated in order. The result is a tri-state integer: `0` (no proof body), `1` (proof body in this source file), `2` (proof body exists but in a different source file — functor-instantiated).

   **Signal 0 — module alias line (functor-instantiated declarations):** If `declared_line` is available, resolve the `.v` source file from `declared_library`. Read the source line at `declared_line`. If the line matches the pattern `Module <ident> :=` (a module application or alias, not a `Module <ident>.` definition block), set `has_proof_body = 2` (done). This detects declarations generated by functor instantiation (`Module I := FloatIntervalFull F`) whose tactic proofs exist in the functor's source file, not in the host file. The regex for module alias lines shall match: optional whitespace, `Module`, whitespace, one or more identifier characters, whitespace, `:=` (i.e., `^\s*Module\s+\w+\s*:=`).

   **Signal 1 — opacity (informational only, no short-circuit):** Opacity from About metadata (§4.1.1) is recorded but does not determine `has_proof_body`. Opaque declarations were proved with `Qed`, but many opaque proofs are auto-generated (HB.instance, Canonical Structure, functor application) and have no extractable tactic script in the `.v` source. Evaluation always continues to Signal 2.
   ...
   **Signal 3 — line-anchored .v source check:** If `declared_line` (from About metadata, §4.1.1) is available, resolve the `.v` source file from `declared_library` using the same prefix-mapping as module path derivation. Read the source line at `declared_line` (1-based). If the line starts with a proof-requiring Vernacular keyword that always enters proof mode (`Lemma`, `Theorem`, `Proposition`, `Corollary`, `Fact`, `Remark`): set `has_proof_body = 1` (done). For ambiguous keywords at the declared line (`Definition`, `Instance`, `Fixpoint`): scan forward from that line for a `Proof` keyword (`Proof.`, `Proof using ...`, `Proof with ...`) before the next declaration keyword; set `has_proof_body = 1` if found. If `declared_line` is unavailable, or the `.v` file does not exist: set `has_proof_body = 0` (conservative default). `.v` file content is cached via a bounded LRU cache (maxsize 16) keyed by source path. Since declarations from the same `.vo` file share a `.v` source and `.vo` files are processed sequentially, the working set is typically 1 entry; the 16-entry bound prevents unbounded memory growth while keeping near-100% hit rates.

> **Given** a declaration `Interval.Tactic.Private.I1.F'.classify_real` of kind `"lemma"` with `declared_line=52`, `declared_library="Interval.Tactic"`, and source line 52 is `Module I1 := FloatIntervalFull F.`
> **When** proof-body detection runs
> **Then** `has_proof_body = 2` (signal 0: module alias line — functor-instantiated)

> **Given** a declaration `Nat.add_comm` with opacity=`"opaque"` (from About) and `declared_line=15`, and source line 15 is `Lemma add_comm : forall n m, n + m = m + n.`
> **When** proof-body detection runs
> **Then** `has_proof_body = 1` (signal 0 does not match; signal 1 is informational only; signal 3: line starts with `Lemma`, a proof-requiring keyword)

> **Given** an opaque declaration `Num.Builders_19.floorP` generated by HB.instance with `declared_line=100`, and source line 100 is `HB.instance Definition _ := Num.IntDomain_isNumDomain.Build R intrP floorP.`
> **When** proof-body detection runs
> **Then** `has_proof_body = 0` (signal 3: line does not start with a proof-requiring or ambiguous keyword; no `Proof` keyword follows)
...
**Process isolation:** The backend shall spawn coq-lsp with `start_new_session=True`. The extraction pipeline runs a single coq-lsp process at a time (no parallel workers). Rocq's coq-lsp double-forks rocqworker processes; these workers do not inherit the coq-lsp process group ID (PGID), environment variables, Unix session ID (SID), or pipe file descriptors — Rocq's internal IPC mechanism severs all observable procfs links between parent and worker. Because the pipeline uses a single backend, all rocqworker processes on the system belong to the current session.

**RSS monitoring:** The pipeline shall monitor the total resident set size (RSS) of the coq-lsp process and all rocqworker processes on the system. The scan proceeds as follows: (1) for each numeric entry in `/proc/`, read `/proc/<pid>/cmdline` and skip entries that do not contain `coq-lsp` or `rocqworker`; (2) for matching entries, read `VmRSS` from `/proc/<pid>/status` and sum. This is correct because the pipeline runs a single coq-lsp backend — all rocqworker processes belong to it. The RSS restart threshold defaults to 5 GiB and is overridable via the `POULE_LSP_RSS_LIMIT` environment variable (in bytes). When RSS exceeds the threshold, the pipeline stops and restarts the backend to reclaim memory. When RSS is below the threshold, the pipeline skips the restart to avoid unnecessary overhead.

**Session cleanup:** On stop, the backend shall kill the coq-lsp process and all rocqworker processes by scanning `/proc/*/cmdline` for processes containing `coq-lsp` or `rocqworker` and sending `SIGKILL` to each match. This broad cleanup is safe because the pipeline runs a single backend at a time. The cleanup shall execute in the `finally` block so it runs even after a clean LSP shutdown sequence — rocqworkers may outlive the main coq-lsp process. The backend shall also attempt `os.killpg(pgid, SIGKILL)` as a belt-and-suspenders measure for any processes that do share the process group.
