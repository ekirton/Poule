...
# ═══════════════════════════════════════════════════════════════════════════
# 4c. fold_val_into_train — Fold Validation Data Back Into Training
# ═══════════════════════════════════════════════════════════════════════════


class TestFoldValIntoTrain:
    """spec §4.1: fold_val_into_train merges val_pairs into train_pairs."""

    def test_merges_train_and_val_pairs(self):
        """Train pairs contain all original train + val pairs."""
        from Poule.neural.training.data import fold_val_into_train

        train = [("train_s1", 0, 0), ("train_s2", 0, 0)]
        val = [("val_s1", 0, 0), ("val_s2", 0, 0), ("val_s3", 0, 0)]
        ds = _make_tactic_dataset(
            train_pairs=train,
            val_pairs=val,
            test_pairs=[("test_s1", 0, 0)],
            train_files=["f1", "f2"],
            val_files=["f3", "f4", "f5"],
            test_files=["f6"],
        )

        result = fold_val_into_train(ds)

        assert len(result.train_pairs) == 5
        assert result.train_pairs == train + val

    def test_val_pairs_empty_after_fold(self):
        """val_pairs is empty after folding."""
        from Poule.neural.training.data import fold_val_into_train
        ...
        assert result.val_pairs == []
        assert result.val_files == []

    def test_test_pairs_unchanged(self):
        """test_pairs and test_files are identical to input."""
        from Poule.neural.training.data import fold_val_into_train

        test = [("test_s1", 0, 0), ("test_s2", 0, 0)]
        test_files = ["tf1", "tf2"]
        ds = _make_tactic_dataset(
            train_pairs=[("t", 0, 0)],
            val_pairs=[("v", 0, 0)],
        ...
        assert result.test_pairs == test
        assert result.test_files == test_files

    def test_train_files_merged(self):
        """train_files is the concatenation of original train + val files."""
        from Poule.neural.training.data import fold_val_into_train

        ds = _make_tactic_dataset(
            train_pairs=[("t1", 0, 0), ("t2", 0, 0)],
            val_pairs=[("v1", 0, 0)],
            test_pairs=[("test", 0, 0)],
            train_files=["f1", "f2"],
            val_files=["f3"],
        ...
        assert result.train_files == ["f1", "f2", "f3"]

    def test_label_maps_preserved(self):
        """Label maps and category fields are copied unchanged."""
        from Poule.neural.training.data import fold_val_into_train
    ...
    def test_no_data_lost_or_duplicated(self):
        """Every pair from train + val appears exactly once in result."""
        from Poule.neural.training.data import fold_val_into_train

        train = [(f"t{i}", 0, 0) for i in range(10)]
        val = [(f"v{i}", 0, 0) for i in range(5)]
        ds = _make_tactic_dataset(
            train_pairs=train,
            val_pairs=val,
            test_pairs=[("test", 0, 0)],
        ...
        assert len(result.train_pairs) == 15
        result_states = [s for s, _, _ in result.train_pairs]
        original_states = [s for s, _, _ in train] + [s for s, _, _ in val]
        assert result_states == original_states
