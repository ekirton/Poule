...
2. Write or update the PRD in `doc/requirements/`.
...
2. Propagate requirements down to `doc/features/`.
3. If a problem with the requirements is detected, do **not** edit requirements — surface the issue to the user and **stop**.
...
3. Propagate features down to `doc/architecture/`.
4. If a problem is found in upstream documents, do **not** edit them — surface the issue to the user and **stop**.
...
4. If a problem is identified with the architecture, write a detailed description to `doc/architecture/feedback/` and **stop**. Notify the user.
...
3. For each feedback item:
   ...
   - If **invalid and the architecture is the problem**: write to `doc/architecture/feedback/`, notify the user, and **stop**.
