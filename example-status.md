...
### Bugs
3. ~~**list_instances returns empty**~~ — FIXED: same root cause as audit_assumptions — `execute_vernacular` did not route through coqtop subprocess. Changed to `send_command(..., prefer_coqtop=True)` per proof-session.md §4.4.1. Spec, tests, and implementation updated.
